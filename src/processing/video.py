...
class VideoCreator:
    ...
    def get_player_color(self, player_id):
        if player_id in self.state.team1.players:
            ...
            return (255, 255, 255)  # Default color if team is not identified
    ...
    def get_player_label(self, player_id, frame_count):
        ...
        for possession in self.state.possessions:
            if possession.start <= frame_count <= possession.end and possession.playerid == player_id:
                return "In Possession"

        # Assign "Shooter" label if the player is attempting a shot
        for shot_attempt in self.state.shot_attempts:
            if shot_attempt.start <= frame_count <= shot_attempt.end and shot_attempt.playerid == player_id:
                return "Shooter"

        return label  # Return the label after checks

    # Highlight possessions on the frame
    def highlight_possessions(self, frame, frame_count):
        ...
        for possession in self.state.possessions:
            if possession.start <= frame_count <= possession.end:
                ...
                if player_id in self.state.frames[frame_count].players:
                    player_frame = self.state.frames[frame_count].players[player_id]
    ...
    def highlight_shot_attempts(self, frame, current_frame):
        ...
        for shot_attempt in self.state.shot_attempts:
            if shot_attempt.start <= current_frame <= shot_attempt.end:
                ...
                game_frame = self.state.frames[current_frame]
    ...
    def run(self):
        ...
        while cap.isOpened():
            ...
            self.highlight_possessions(frame, frame_count)
            self.highlight_shot_attempts(frame, frame_count)

            # Get the game frame data for the current frame count
            game_frame = (
                self.state.frames[frame_count]
                if frame_count < len(self.state.frames)
                else None
            )
            ...
            if game_frame:
                ...
                for player_id, player_frame in game_frame.players.items():
                    player_label = self.get_player_label(player_id, frame_count)  # Determine label
                    player_color = self.get_player_color(player_id)  # Determine color
                    self.draw_boxes(frame, [player_frame.box], player_color, label=player_label)
                ...
                if game_frame.ball:
                    ...
                    self.draw_boxes(frame, [game_frame.ball.box], self.BALL_COLOR, label=ball_label)
